...
def MAKENULL() :
	...
	for i in range(B) :
		tmp.append(None)	# I probably could use `tmp = [None]*B` but I had
							# issues where all the elements maintained the same
							# data before, hence the for
							# loop.
	return tmp
...
def LOCATE(word, D) :
	...
	initial = h(word)
	i = 0
	while ( i < B ) and ( D[ (initial + i) % B ] != word ) and ( D[ (initial
+ i) % B ] is not None ) :
		i += 1
	g_probeCount += i
	return  (initial + i) % B
...
# inserts a word into the appropriate bucket in D, provided the word is not
# already there
...
# deletes a word from its bucket in D.  Does not test if it exists first, as
# that is lossy
